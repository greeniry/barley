load(paste(getwd(),"/support/info+data.RData",sep=""))
library(readxl)
library(ggplot2)
...
id=id[-1]
# data=log(data)
wtcd=apply(data[id,28:30],2,sum)
wtbl=apply(data[id,25:27],2,sum)
adcd=apply(data[id,7:12],2,sum)
adbl=apply(data[id,1:6],2,sum)
# wtcd=data[id,37:42]
# wtbl=data[id,31:36]
# adcd=data[id,19:24][-3]
# adbl=data[id,13:18]
wilcox.test(wtbl,wtcd,correct = F,exact = F)[["p.value"]]
wilcox.test(adbl,adcd,correct = F,exact = F)[["p.value"]]
rcompanion::scheirerRayHare(value~type+feed,data=data.frame(
  ...
  value=c(wtcd,wtbl,adcd,adbl)))
{
  
  t1<-summary(aov(data~type,data=data.frame(data=c(wtbl,wtcd),type=c(rep("1",3),rep("2",3)))))[[1]][["Pr(>F)"]][1]
  t2<-summary(aov(data~type,data=data.frame(data=c(adcd,adbl),type=c(rep("1",6),rep("2",6)))))[[1]][["Pr(>F)"]][1]
  t3<-summary(aov(data~type,data=data.frame(data=c(adbl,wtcd),type=c(rep("1",6),rep("2",3)))))[[1]][["Pr(>F)"]][1]
