load(paste(substr(getwd(),1,stringr::str_length(getwd())-2),"/support/umap_naive.RData",sep=""))
load(paste(substr(getwd(),1,stringr::str_length(getwd())-2),"/support/del_new_rmk.RData",sep=""))

library(ggplot2)
pdata<-data.frame(ID=unique(new_info_nao$sample),
                  Type=c("ADBL","ADBL","WTCD","ADBL","ADCD","WTCD","ADCD","ADCD","ADBL","WTCD"),
                  Sum=0,jiaozhi=0,jiaozhi_baifenbi=0)
for (i in 1:10)
{
  pdata[i,4]=length(which(new_info_nao$sample==pdata[i,1]&new_info_nao$group3=="Microglia"))
  pdata[i,3]=length(which(info_nao$sample==pdata[i,1]))
  # pdata[i,3]=pdata[i,4]+pdata[i,5]
  pdata[i,5]=pdata[i,4]/pdata[i,3]*100
  pdata[i,4]=log(pdata[i,4])
}

{adbl<-pdata$jiaozhi_baifenbi[which(pdata$Type=="ADBL")]
  adcd<-pdata$jiaozhi_baifenbi[which(pdata$Type=="ADCD")]
  wtcd<-pdata$jiaozhi_baifenbi[which(pdata$Type=="WTCD")]
  ...
  P <- ggplot(picdata,aes(x=Group,y=mdata))+ 
    ...
    ggtitle("feijiaozhixibao")+ #设置总的标题
    theme_bw()+ #背景变为白色
