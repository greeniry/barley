...
pdata<-data.frame(ID=unique(jiaozhi_info_nao$sample),
                  ...
                  DAM_baifenbi=0,activated=0,Homeostatic=0)
for (i in 1:10)
{
  pdata[i,3]=length(intersect(which(jiaozhi_info_nao$sample==pdata[i,1]),DAM_id))/
    length(intersect(which(jiaozhi_info_nao$sample==pdata[i,1]),activated_id))
  pdata[i,4]=length(intersect(which(jiaozhi_info_nao$sample==pdata[i,1]),activated_id))/
    length(which(jiaozhi_info_nao$sample==pdata[i,1]))
  pdata[i,5]=length(intersect(which(jiaozhi_info_nao$sample==pdata[i,1]),Homeostatic_id))/
    length(which(jiaozhi_info_nao$sample==pdata[i,1]))
}
{adbl<-pdata$DAM_baifenbi[which(pdata$Type=="ADBL")]
  adcd<-pdata$DAM_baifenbi[which(pdata$Type=="ADCD")]
  wtcd<-pdata$DAM_baifenbi[which(pdata$Type=="WTCD")]
