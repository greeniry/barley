# This is support code to generate Bcell_30_umap.RData, CD4Tcell_10_umap.RData,
# CD8Tcell_5_umap.RData, MDM_421.RData and rmk_umap.RData.
...
load("C:/Users/li08/Desktop/barley/cytof/serum/rmk_umap.RData")
library(umap)
library(ggplot2)
rm(cl_i,nao_umap)
{
  del_id=c(11,21,38,1,2)
  data=data_serum[which(info_serum$group4=="CD8+T cells"),]
  info=info_serum[which(info_serum$group4=="CD8+T cells"),]
  ...
  centers=5
  {
    ...
    p1<-ggplot(tsne_result[,],aes(umap1,umap2,color=as.character(cl_i$cluster))) +
      geom_point() +scale_color_manual(values = colorset[1:centers])
    # ggsave(paste("/data/yuzhou/serum0807/",
    #              "CD8Tcell_",centers,"_umap.png",sep=""),
    #        p1,units="in",width = 10,height = 10,dpi=300)
    
    pdata<-data[1,]
    for (i in 1:centers)
    {
      pdata<-rbind(pdata,apply(data[which(cl_i$cluster==i),],2,mean))
      rownames(pdata)[i+1]=paste("Cluster_",i,sep="")
    }
    pdata=pdata[-1,]
    library(pheatmap)
    # pheatmap(pdata[,-del_id],scale = "row",show_colnames = T,show_rownames = T,
    #          color = colorRampPalette(c("#055070", "white","#ae1266"))(50),
    #          annotation_names_row = TRUE,cluster_cols = F,cluster_rows = T,
    #          filename = paste("/data/yuzhou/serum0807/",
    #                           "CD8Tcell_",centers,"_umap_heatmap.pdf",sep=""),
    #          width=10,height = 2+centers*0.5)
    pdata<-data.frame(A=rep(0,centers),B=rep(0,centers),C=rep(0,centers))
    for (i in 1:centers)
    {
      pdata$A[i]=length(which(info$group3=="B_A"&cl_i$cluster==i))
    ...
    # write.csv(pdata,paste("/data/yuzhou/serum0807/",
    #                       "CD8Tcell_",centers,"_umap.csv",sep=""))
    # save.image(paste("/data/yuzhou/serum0807/",
    #                  "CD8Tcell_",centers,"_umap.RData",sep=""))
    
    save.image("C:/Users/li08/Desktop/barley/cytof/serum/CD8Tcell_5_umap.RData")
  }
}
...
library(ggplot2)
rm(cl_i,nao_umap)
# id=which(tsne_result$umap1>5|(tsne_result$umap2<45&tsne_result$umap2>-16))
del_id=c(21,20,40,39,1,2,3,4)
#cd4
{
  data=data[,-del_id]
  info=info[,]
  nao_umap<- umap(data[,],preserve.seed = TRUE, method = c("naive"))
  # data=data[which(nao_umap$layout[,1]<20&nao_umap$layout[,2]>-5),]
  # info=info[which(nao_umap$layout[,1]<20&nao_umap$layout[,2]>-5),]
  # nao_umap$layout=nao_umap$layout[which(nao_umap$layout[,1]<20&nao_umap$layout[,2]>-5),]
  centers=10
  {
    tsne_result = data.frame(nao_umap$layout)
    colnames(tsne_result) = c("umap1","umap2")
    cl_i <- kmeans(data[,], centers = centers, nstart = 3,algorithm="Lloyd",iter.max =50)
    colorset=c(
      "#ffff99","#b15928","#8dd3c7","#bebada","#fb8072",
      "#80f1f3","#67001f","#fccde5","#bc80ed","#1c808d",
      "#0571b0","#3c0030","#303c00","#00671f","#bf2000",
    ...
    p1<-ggplot(tsne_result[,],aes(umap1,umap2,color=as.character(cl_i$cluster))) +
      geom_point() +scale_color_manual(values = colorset[1:centers])
    # ggsave(paste("/data/yuzhou/serum0812/",
    #              "CD4Tcell_",centers,"_umap.png",sep=""),
    #        p1,units="in",width = 10,height = 10,dpi=300)
    ...
    #          color = colorRampPalette(c("#055070", "white","#ae1266"))(50),
    #          annotation_names_row = TRUE,cluster_cols = F,cluster_rows = T,
    #          filename = paste("/data/yuzhou/serum0812/",
    #                           "CD4Tcell_",centers,"_umap_heatmap.pdf",sep=""),
    #          width=10,height = 2+centers*0.5)
    pdata<-data.frame(A=rep(0,centers),B=rep(0,centers),C=rep(0,centers))
    for (i in 1:centers)
    {
    ...
    # write.csv(pdata,paste("/data/yuzhou/serum0812/",
    #                       "CD4Tcell_",centers,"_umap.csv",sep=""))
    # save.image(paste("/data/yuzhou/serum0812/",
    #                  "CD4Tcell_",centers,"_umap.RData",sep=""))
    
    save.image("C:/Users/li08/Desktop/barley/cytof/serum/CD4Tcell_10_umap.RData")
  }
}
...
{
  del_id=c(11,21,38,1,2)
  data=data_serum[which(info_serum$group4=="Bcell"),]
  info=info_serum[which(info_serum$group4=="Bcell"),]
  nao_umap<- umap(data[,-del_id],preserve.seed = TRUE, method = c("naive"))
  # data=data[which(nao_umap$layout[,1]<20&nao_umap$layout[,2]>-5),]
  # info=info[which(nao_umap$layout[,1]<20&nao_umap$layout[,2]>-5),]
  # nao_umap$layout=nao_umap$layout[which(nao_umap$layout[,1]<20&nao_umap$layout[,2]>-5),]
  centers=30
  {
    ...
    #          annotation_names_row = TRUE,cluster_cols = F,cluster_rows = T,
    #          filename = paste("/data/yuzhou/serum0807/",
    #                           "Bcell_",centers,"_umap_heatmap.pdf",sep=""),
    #          width=10,height = 2+centers*0.5)
    pdata<-data.frame(A=rep(0,centers),B=rep(0,centers),C=rep(0,centers))
    for (i in 1:centers)
    {
    ...
    # write.csv(pdata,paste("/data/yuzhou/serum0807/",
    #                       "Bcell_",centers,"_umap.csv",sep=""))
    # save.image(paste("/data/yuzhou/serum0807/",
    #                  "Bcell_",centers,"_umap.RData",sep=""))
    
    save.image("C:/Users/li08/Desktop/barley/cytof/serum/Bcell_30_umap.RData")
  }
}
