library(ggplot2)
library(readxl)
onehot<-read_xlsx(paste(getwd(),"/support/class_onehot.xlsx",sep=""),sheet=1)
...
pic_4<-data.frame(name=deFA$name,fc=log(deFA$fc),pvalue=-log10(deFA$pvalue),
                  color=c(rep("SCFA",6),rep("MCFA",6),rep("LCFA",38-12)))
pic_4<-pic_4[-27,]
pic_4de<-pic_4[which(pic_4$pvalue>3),]
pic_4de$name[10]="***-octadecatrienoic acid"
ggplot()+geom_point(data=pic_4,mapping = aes(x=fc,y=pvalue,color=color,size=abs(fc)))+
