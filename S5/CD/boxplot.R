...
{
  data=ldata[[batch]]
  data=cbind(data[,1:7],data[,which(substr(colnames(data),15,15)=="6")])
  pdata=data.frame(
  ...
  for (i in 1:2)
  {
    ...
    t3<-rcompanion::scheirerRayHare(value~type+feed,data=data.frame(
      type=factor(c(rep("wt",16),rep("ad",15)),levels = c("wt","ad")),
      feed=factor(c(rep("cd",6),rep("bl",10),rep("cd",5),rep("bl",10)),levels = c("cd","bl")),
      group=factor(c(rep("wtcd",6),rep("wtbl",10),rep("adcd",5),rep("adbl",10)),
