...
library(ggplot2)
id=unique(info_serum$group4)
for (j in c(1,2,5,6,8))
{
  pdata<-data.frame(ID=info_serum$sample[which(!duplicated(info_serum$sample))],
                    Type=info_serum$group3[which(!duplicated(info_serum$sample))],
                    Sum=0,type=0,type_baifenbi=0)
  for (i in 1:12)
  {
    pdata[i,4]=length(which(info_serum$sample==pdata[i,1]&info_serum$group4==id[j]))
    pdata[i,3]=length(which(info_serum$sample==pdata[i,1]))
    pdata[i,5]=pdata[i,4]/pdata[i,3]*100
  }
  pdata=pdata[-which(pdata$ID=="BM0010"|pdata$ID=="BM0008"),]
  {
    adbl<-pdata$type_baifenbi[which(pdata$Type=="B_C")]
    adcd<-pdata$type_baifenbi[which(pdata$Type=="B_B")]
    wtcd<-pdata$type_baifenbi[which(pdata$Type=="B_A")]
    ...
    ggsave(paste(getwd(),"/",id[j],".pdf",sep=""),
