library(readxl)
library(ggplot2)
data<-read_xlsx(paste(getwd(),"/support/血清_all.xlsx",sep=""),sheet = 1)
data<-as.matrix(data[,-c(1:4)])
info=read_xlsx(paste(getwd(),"/support/xueqing_info.xlsx",sep=""))
scfainfo=read_xlsx(paste(getwd(),"/support/血清SCFAs分类.xlsx",sep=""))
id=0
for (i in 1:12)
{
  id=c(id,which(info$Description==scfainfo$`1`[i]))
}
id=id[-1]
data=log(data)
wtcd=apply(data[id,42:47],2,sum)
wtbl=apply(data[id,36:41],2,sum)
adcd=apply(data[id,24:29],2,sum)
adbl=apply(data[id,18:23],2,sum)
...
id=0
scfainfo=scfainfo[which(scfainfo$`2`=="丁酸"),]
for (i in 1:12)
{
  id=c(id,which(info$Description==scfainfo$`1`[i]))
}
