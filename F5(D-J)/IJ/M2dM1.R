...
for (i in 1:12)
{
  pdata[i,3]=length(which(info$sample==pdata[i,1]&cl_i$cluster==1))/length(which(info$sample==pdata[i,1]&cl_i$cluster==2))
}
pdata=pdata[-c(6,8),]
{adbl<-pdata$M2DM1[which(pdata$Type=="B_C")]
  adcd<-pdata$M2DM1[which(pdata$Type=="B_B")]
  wtcd<-pdata$M2DM1[which(pdata$Type=="B_A")]
  print(mean(adcd))
  print(sd(adcd))
  print(mean(adbl))
  print(sd(adbl))
...
for (i in 1:12)
{
  pdata[i,3]=length(which(info$sample==pdata[i,1]&cl_i$cluster==1))
}
pdata=pdata[-c(6,8),]
